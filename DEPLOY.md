...
- Домены выбраны заранее и захардкожены в конфигах, чтобы не было проблемы
  ...
  - Backend: `https://ai-survey-backend.onrender.com`
  - Frontend: `https://ai-survey-frontend.vercel.app`

Если любое из этих имён окажется занято при деплое — см. раздел
"Если имя занято" в конце файла.

## Данные пока хранятся in-memory (по решению пользователя)
...
запросу (~30–60 сек, holodный старт) — при этом все сессии, что были в
памяти, теряются. Для демо это ок; если позже понадобится персистентность —
это будет отдельная задача (например, Postgres на Render/Neon).
...
5. Проверить: `curl https://ai-survey-backend.onrender.com/api/health` →
...
- `rewrites` — проксирует все запросы `/api/*` на
  `https://ai-survey-backend.onrender.com/api/*`, поэтому фронтенд-код
...
После деплоя Vercel выдаст домен вида `ai-survey-frontend.vercel.app` —
...
1. Открыть `https://ai-survey-frontend.vercel.app`.
...
- Персистентная БД для сессий (Postgres на Render/Neon), если нужно
  переживать холодные старты без потери данных.
