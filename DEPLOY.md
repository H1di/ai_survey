...
> **Сервис уже задеплоен** как `ai-survey-backend-3g62`
> (`https://ai-survey-backend-3g62.onrender.com`). `render.yaml` содержит именно
> это имя, поэтому Blueprint будет **обновлять** существующий сервис, а не
> создавать новый. Шаги ниже нужны только для деплоя с нуля.
...
2. Render найдёт `render.yaml` в корне и предложит создать сервис `ai-survey-backend-3g62`.
